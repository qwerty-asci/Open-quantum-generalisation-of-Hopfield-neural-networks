...
class Red():
    ...
    def overlap(self,M):
        ...
        for j in range(self.n):
            m+=self.red[j]*np.trace(self.sz()[j]@M)
    ...
    def overlapy(self,O):
        ...
        m=0.0
        for j in range(self.n):
            m+=self.red[j]*np.trace(self.sy()[j]@O)
...
if __name__=="__main__":
    ...
    R=Red(qubits,semilla,1,omega,beta,transicion)
    ...
    #Sacamos la matriz densidad que vamos a hacer evolucionar
    
    M=O.matriz_densidad().copy()
    
    
    m_totalz_T=np.zeros((int(int(T/h)/4)),dtype=np.float64)
    m_totaly_T=np.zeros((int(int(T/h)/4)),dtype=np.float64)
    traza=np.zeros((int(int(T/h)/4)),dtype=np.complex128)
    
    #Vamos a calcular la magnetización en z e y junto con la traza para ver que la simulación se está realizando correctamente
    t=0.0
    S=np.zeros((2**qubits,2**qubits))
    for cont in range(int(T/h)):
        
        print(cont)
        if(cont%4==0):
            m_totalz_T[int(cont/4)]=np.real(R.overlap(M))
            m_totaly_T[int(cont/4)]=np.real(R.overlapy(M))
            traza[int(cont/4)]=np.trace(M)
        
        S=-1.0j*(R.Heff()@M-M@Operador.hermitico(R.Heff()))
        
        for i in range(qubits):
            S+=R.saltomas()[i]@M@Operador.hermitico(R.saltomas()[i])+R.saltomenos()[i]@M@Operador.hermitico(R.saltomenos()[i])
    ...
    np.savetxt("mzT.dat",m_totalz_T)
    np.savetxt("myT.dat",m_totaly_T)
    np.savetxt("traza.dat",traza)
