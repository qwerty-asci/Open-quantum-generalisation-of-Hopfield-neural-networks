...
if __name__=="__main__":
    ...
    R=Red(qubits,semilla,patrones,omega,beta[0],transicion)
    ...
    with open("overlap.dat","w") as Fichero:
        for q in range(columnas):
            ...
            R=Red(qubits,semilla,patrones,omega,beta[q],transicion)
            ...
            if(signo1!=0):
                m_totalz[:,q]=m_totalz[:,q]
                m_totaly[:,q]=m_totaly[:,q]
            if(signo2!=0):
                m_totalz_menos[:,q]=m_totalz_menos[:,q]
                m_totaly_menos[:,q]=m_totaly_menos[:,q]
            
            m_totalz_T[:,q]=m_totalz_T[:,q]
            m_totaly_T[:,q]=m_totaly_T[:,q]
