...
class Onda():
    ...
    def __init__(self,n_qubit,semilla,pesos=0.0):
...
class Red():
    def __init__(self,qubits,semilla,patrones,omega,beta,gamma):
        ...
        """
            ...
            gm: nos da la fuerza que tienen los operadores de gm
...
class Excepcion_limite(Exception):
...
if __name__=="__main__":
    ...
    #Creamos la red e inicializamos la funcion de de onda
    R=Red(qubits,semilla,patrones,omega,beta,gm)
    O=Onda(qubits,semilla)
    
    rng=np.random.default_rng(5674543)
    
    
    sz0=rng.uniform(-0.05,0.05,columnas)
    sy0=rng.uniform(-0.05,0.05,columnas)
    sx0=rng.uniform(-0.05,0.05,columnas)
    ...
    mx=np.zeros((int(T/(guardado*h)),sz0.size),np.float128)
    ...
    try:
        for w in range(sz0.size):
            ...
            for i in range(int(T/h)):
                ...
                # suma_auxiliar1=np.zeros((2**qubits,2**qubits),dtype=np.complex128)
                # auxiliar3=np.zeros((2**qubits,2**qubits),dtype=np.complex128)
                suma_auxiliar1=0.0
                for j in range(len(R.red)):
                #     # suma_auxiliar1+=R.red[j]*Operador().funcion_sobre_operador(sz*beta*R.red[j],np.tanh)*gm/qubits
                    suma_auxiliar1+=R.red[j]*np.tanh(sz*beta*R.red[j])*gm/qubits
                # # breakpoint()
                # auxiliar2=sz.copy()
                auxiliar2=sz
                derivada_sz=2.0*omega*sy+suma_auxiliar1-gm*sz+2.0*w*sx
                
                # # print(derivada_sz)
                
                # # for j in range(len(R.red)):
                # #     auxiliar3+=Operador().funcion_sobre_operador(sz*beta*R.red[j],np.tanh)*gm*derivada_sz*beta/qubits
                
                # #Actualizo sz
                sz+=h*derivada_sz#+h**2/2.0*(-4.0*omega**2*sz-gm*omega*sy+auxiliar3-gm*derivada_sz)
                # #Actualizo sy
                sy+=h*(2.0*omega*auxiliar2-gm*sy/2.0)+h**2/2.0*(2.0*omega*derivada_sz+gm*omega*auxiliar2+gm**2*sy/2.0)
                sx+=h*(-2.0*w*auxiliar2-gm*sy/2.0)+h**2/2.0*(-2.0*w*derivada_sz+gm*omega*auxiliar2+gm**2*sy/2.0)
                
                if(i%guardado==0):
                    # mz[int(i/guardado)]=np.real(O.valor_esperado(sz)[0,0])
                    # my[int(i/guardado)]=np.real(O.valor_esperado(sy)[0,0])
                    mz[int(i/guardado),w]=sz
                    my[int(i/guardado),w]=sy
                    mx[int(i/guardado),w]=sx
                    # breakpoint()
                    
                    if(np.abs(mz[int(i/guardado),w])>1.1 or np.abs(my[int(i/guardado),w])>1.1):
                        raise Excepcion_limite(int(i/guardado))
    except Excepcion_limite as EL:
        print("Se han roto las condiciones de contorno")
        np.savetxt("mz.dat",mz[0:EL.cuenta,:])
        np.savetxt("my.dat",my[0:EL.cuenta,:])
    # breakpoint()
    
    
    np.savetxt("mz.dat",mz)
    np.savetxt("my.dat",my)
    np.savetxt("mx.dat",mx)
